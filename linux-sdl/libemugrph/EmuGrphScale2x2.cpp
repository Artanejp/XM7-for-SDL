...
void EmuGrphScale2x2::SetVramReader(void p(Uint32, Uint32 *, Uint32), int w, int h)
{
	getvram = p;
	vramwidth = w;
	vramheight = h;
...
void EmuGrphScale2x2::SetPutWord(void p(Uint32 *, Uint32, Uint32 *))
{
	putword = p;
}

void EmuGrphScale2x2::SetConvWord(void p(SDL_Surface *, Uint32 *, Uint32 *))
{
	convword = p;
}
