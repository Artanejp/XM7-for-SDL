 ...
 * テープスピード状態の連動
 */
static void FASTCALL OnGP_TAPESPEEDClicked(GtkWidget *widget, gpointer data)
{
 ...
 * テンキー状態の連動
 */
static void FASTCALL OnKP_USEARROWFOR10Clicked(GtkWidget *widget, gpointer data)
{
...
GtkWidget* FASTCALL OpenPropertyPage(void)
{
  ...
  char stmp[64];
	...
	GP_HIGHSPEED = gtk_radio_button_new_with_mnemonic (NULL, "高速(FM-77以降)");
	gtk_widget_show (GP_HIGHSPEED);
	gtk_box_pack_start (GTK_BOX (vbox4), GP_HIGHSPEED, FALSE, FALSE, 0);
	gtk_radio_button_set_group (GTK_RADIO_BUTTON (GP_HIGHSPEED), GP_HIGHSPEED_group);
	GP_HIGHSPEED_group = 
		gtk_radio_button_get_group (GTK_RADIO_BUTTON (GP_HIGHSPEED));

	GP_LOWSPEED = gtk_radio_button_new_with_mnemonic (NULL, "低速(FM-7互換)");
	gtk_widget_show (GP_LOWSPEED);
	gtk_box_pack_start (GTK_BOX (vbox4), GP_LOWSPEED, FALSE, FALSE, 0);
	gtk_radio_button_set_group (GTK_RADIO_BUTTON (GP_LOWSPEED), GP_HIGHSPEED_group);
	GP_HIGHSPEED_group = gtk_radio_button_get_group (GTK_RADIO_BUTTON (GP_LOWSPEED));
...
#if XM7_VER >= 3
	GP_FASTMMR = gtk_menu_item_new_with_mnemonic ("MMR高速モード時");
	...
	GP_CPUTEXT = gtk_spin_button_new (GTK_ADJUSTMENT (GP_CPUTEXT_adj), 1, 0);
	...
	GP_TAPESPEED =
		gtk_check_button_new_with_mnemonic ("テープモータオン時はフルスピード動作");
	...
	GP_TAPESPEEDMODE =
		gtk_check_button_new_with_mnemonic ("テープモータオン時の動作を速度調整省略にする");
	gtk_widget_show (GP_TAPESPEEDMODE);
	gtk_box_pack_start (GTK_BOX (vbox5), GP_TAPESPEEDMODE, FALSE, FALSE, 0);

#ifdef FDDSND
	GP_FDDWAIT =
		gtk_check_button_new_with_mnemonic ("フロッピーアクセス時にウェイトを挿入する");
	...
	GP_AUTOSPEEDADJUST = gtk_check_button_new_with_mnemonic ("自動速度調整");
	gtk_widget_show (GP_AUTOSPEEDADJUST);
	gtk_box_pack_start (GTK_BOX (hbox3), GP_AUTOSPEEDADJUST, TRUE, FALSE, 0);
	gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (GP_AUTOSPEEDADJUST), TRUE);
	...
	gtk_notebook_set_tab_label (GTK_NOTEBOOK (propertySheet), 
		gtk_notebook_get_nth_page (GTK_NOTEBOOK (propertySheet), 0), lblGeneral);
	...
	SP_NONE = gtk_radio_button_new_with_mnemonic (NULL, "合成しない");
	...
	gtk_frame_set_label_widget (GTK_FRAME (frmSamplingrate), lblSamplingrate);
	...
	SP_STEREOQ_REV = gtk_menu_item_new_with_mnemonic ("ステレオ(反転)");
	gtk_widget_show (SP_STEREOQ_REV);
	gtk_container_add (GTK_CONTAINER (menu3), SP_STEREOQ_REV);

	SP_STEREOQ_THG = gtk_menu_item_new_with_mnemonic ("ステレオ(THG)");
	...
	SP_BEEPSPIN = gtk_spin_button_new (GTK_ADJUSTMENT (SP_BEEPSPIN_adj), 1, 0);
	...
	SP_TAPEMON = gtk_check_button_new_with_mnemonic ("テープ音モニタ");
	gtk_widget_show (SP_TAPEMON);
	gtk_box_pack_start (GTK_BOX (vbox9), SP_TAPEMON, FALSE, FALSE, 0);

#ifdef FDDSND
	SP_FDDSOUND = gtk_check_button_new_with_mnemonic ("FDDシーク音/CMTリレー音");
	...
	gtk_notebook_set_tab_label (GTK_NOTEBOOK (propertySheet),
		gtk_notebook_get_nth_page (GTK_NOTEBOOK (propertySheet), 1), lblSound);
	...
	KP_USEARROWFOR10 =
		gtk_check_button_new_with_mnemonic ("カーソルキーをテンキーに対応させる");
	gtk_widget_show (KP_USEARROWFOR10);
	gtk_box_pack_start (GTK_BOX (vbox10), KP_USEARROWFOR10, FALSE, FALSE, 0);

	KP_ARROW8DIR =
		gtk_check_button_new_with_mnemonic ("カーソルキーの2つ同時押しを斜め方向に対応させる");
	...
	KP_KBDREAL = gtk_check_button_new_with_mnemonic ("擬似リアルタイムキースキャン");
	...
	JOY1_group = gtk_radio_button_get_group (GTK_RADIO_BUTTON (JOY1_UNUSED));
	...
	JOY2_group = gtk_radio_button_get_group (GTK_RADIO_BUTTON (JOY2_UNUSED));
	...
	gtk_notebook_set_tab_label (GTK_NOTEBOOK (propertySheet),
	gtk_notebook_get_nth_page (GTK_NOTEBOOK (propertySheet), 3), lblJoyStick1);
	...
	gtk_box_pack_start (GTK_BOX (vbox23[j]), lblJoyrapidButton[j][i], FALSE, FALSE, 0);
	gtk_label_set_justify (GTK_LABEL (lblJoyrapidButton[j][i]), GTK_JUSTIFY_LEFT);


          /* 連射関連 */
                 JOY_RAPID[j][i] = gtk_option_menu_new ();
	gtk_widget_show (JOY_RAPID[j][i]);
	gtk_box_pack_start (GTK_BOX (vbox23[j]), JOY_RAPID[j][i], FALSE, FALSE, 0);

	menu_js_rapid[j][i] = gtk_menu_new ();

	JOY_RAPID0[j][i] = gtk_menu_item_new_with_mnemonic ("連射なし");
	gtk_widget_show (JOY_RAPID0[j][i]);
	gtk_container_add (GTK_CONTAINER (menu_js_rapid[j][i]), JOY_RAPID0[j][i]);

	JOY_RAPID1[j][i] = gtk_menu_item_new_with_mnemonic ("1連射");
	gtk_widget_show (JOY_RAPID1[j][i]);
	gtk_container_add (GTK_CONTAINER (menu_js_rapid[j][i]), JOY_RAPID1[j][i]);

	JOY_RAPID2[j][i] = gtk_menu_item_new_with_mnemonic ("2連射");
	gtk_widget_show (JOY_RAPID2[j][i]);
	gtk_container_add (GTK_CONTAINER (menu_js_rapid[j][i]), JOY_RAPID2[j][i]);

	JOY_RAPID3[j][i] = gtk_menu_item_new_with_mnemonic ("3連射");
	gtk_widget_show (JOY_RAPID3[j][i]);
	gtk_container_add (GTK_CONTAINER (menu_js_rapid[j][i]), JOY_RAPID3[j][i]);

	JOY_RAPID4[j][i] = gtk_menu_item_new_with_mnemonic ("4連射");
	gtk_widget_show (JOY_RAPID4[j][i]);
	gtk_container_add (GTK_CONTAINER (menu_js_rapid[j][i]), JOY_RAPID4[j][i]);

	JOY_RAPID5[j][i] = gtk_menu_item_new_with_mnemonic ("5連射");
	gtk_widget_show (JOY_RAPID5[j][i]);
	gtk_container_add (GTK_CONTAINER (menu_js_rapid[j][i]), JOY_RAPID5[j][i]);
          
	JOY_RAPID6[j][i] = gtk_menu_item_new_with_mnemonic ("6連射");
	gtk_widget_show (JOY_RAPID6[j][i]);
	gtk_container_add (GTK_CONTAINER (menu_js_rapid[j][i]), JOY_RAPID6[j][i]);

	JOY_RAPID8[j][i] = gtk_menu_item_new_with_mnemonic ("8連射");
	gtk_widget_show (JOY_RAPID8[j][i]);
	gtk_container_add (GTK_CONTAINER (menu_js_rapid[j][i]), JOY_RAPID8[j][i]);

	JOY_RAPID12[j][i] = gtk_menu_item_new_with_mnemonic ("12連射");
	gtk_widget_show (JOY_RAPID12[j][i]);
                 gtk_container_add (GTK_CONTAINER (menu_js_rapid[j][i]), JOY_RAPID12[j][i]);

	JOY_RAPID25[j][i] = gtk_menu_item_new_with_mnemonic ("25連射");
	gtk_widget_show (JOY_RAPID25[j][i]);
	gtk_container_add (GTK_CONTAINER (menu_js_rapid[j][i]), JOY_RAPID25[j][i]);

	gtk_option_menu_set_menu (GTK_OPTION_MENU (JOY_RAPID[j][i]), menu_js_rapid[j][i]);
          }
                 sprintf(stmp, "#%d", j);
	...
	gtk_notebook_set_tab_label (GTK_NOTEBOOK (propertySheet),
	gtk_notebook_get_nth_page (GTK_NOTEBOOK (propertySheet), 4), lblJoyStick2);
	...
	gtk_notebook_set_tab_label (GTK_NOTEBOOK (propertySheet),
		gtk_notebook_get_nth_page (GTK_NOTEBOOK (propertySheet), 5), lblScreen2);
	...
	OP_MOUSEEM = gtk_check_button_new_with_mnemonic ("マウスを使用する");
	...
	OP_MOUSE_PORT1 = gtk_radio_button_new_with_mnemonic (NULL, "ポート1にマウスを接続");
	...
	gtk_radio_button_set_group (GTK_RADIO_BUTTON (OP_MOUSE_PORT1), OP_MOUSE_PORT1_group);
	OP_MOUSE_PORT1_group = gtk_radio_button_get_group (GTK_RADIO_BUTTON (OP_MOUSE_PORT1));

	OP_MOUSE_PORT2 = gtk_radio_button_new_with_mnemonic (NULL, "ポート2にマウスを接続");
	...
	gtk_radio_button_set_group (GTK_RADIO_BUTTON (OP_MOUSE_PORT2), OP_MOUSE_PORT1_group);
	OP_MOUSE_PORT1_group = gtk_radio_button_get_group (GTK_RADIO_BUTTON (OP_MOUSE_PORT2));
	...
	gtk_notebook_set_tab_label (GTK_NOTEBOOK (propertySheet),
                                    gtk_notebook_get_nth_page (GTK_NOTEBOOK (propertySheet), 6), lblOption);
	...
	gtk_signal_connect ((gpointer) GP_CPUDEFAULT,
		"clicked", GTK_SIGNAL_FUNC(OnGP_CPUDEFAULTClicked), NULL);

	gtk_signal_connect ((gpointer) GP_CPUCOMBO,
		"changed", GTK_SIGNAL_FUNC(OnGP_CPUCOMBOChanged), NULL);

	gtk_signal_connect ((gpointer) GP_TAPESPEED,
		"clicked", GTK_SIGNAL_FUNC(OnGP_TAPESPEEDClicked), NULL);

	gtk_signal_connect ((gpointer) KP_USEARROWFOR10,
		"clicked", GTK_SIGNAL_FUNC(OnKP_USEARROWFOR10Clicked), NULL);

	gtk_signal_connect ((gpointer) btnOk,
		"clicked", GTK_SIGNAL_FUNC(OnConfig_OK), NULL);
