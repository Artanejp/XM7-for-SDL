...
INLINE void
WRITEB(cpu6809_t * t, WORD addr, BYTE data)
{
...
INLINE void
WRITEW(cpu6809_t * t, WORD addr, WORD data)
{
...
INLINE WORD
RM16(cpu6809_t * m68_state, WORD Addr)
{
...
INLINE void
WM16(cpu6809_t * m68_state, WORD Addr, WORD data)
{
...
static void
UpdateState(cpu6809_t * m68_state)
{
...
static void
cpu_execline(cpu6809_t * m68_state)
{
	BYTE ireg;
//      debugger_instruction_hook(device, PCD);
	...
	switch (ireg) {
		...
		case 0x87:
			flag8_im(m68_state);
			break;	/* undocumented *///sta_im(m68_state);    break;
		case 0x88:
			eora_im(m68_state);
			break;
		case 0x89:
		...
		case 0x8f:
			flag16_im(m68_state);
			break;	/* undocumented *///stx_im(m68_state);    break;
		case 0x90:
			suba_di(m68_state);
			break;
		case 0x91:
		...
		case 0xc7:
			flag8_im(m68_state);
			break;	/* undocumented *///stb_im(m68_state);    break;
		case 0xc8:
			eorb_im(m68_state);
			break;
		case 0xc9:
		...
		case 0xcd:
			trap(m68_state);
			break;	/* undocumented *///std_im(m68_state);    break;
		case 0xce:
			ldu_im(m68_state);
			break;
		case 0xcf:
			flag16_im(m68_state);
			break;	/* undocumented *///stu_im(m68_state);    break;
		case 0xd0:
			subb_di(m68_state);
			break;
		case 0xd1:
...
INLINE void
fetchsub_IDX(cpu6809_t * m68_state, WORD upper, WORD lower)
{
	int indirect = 0;
	WORD *reg;
	BYTE b;
...
INLINE void
fetch_effective_address(cpu6809_t * m68_state)
{
	BYTE postbyte;
	WORD upper, lower;
...
fetch_effective_address(cpu6809_t * m68_state)
{
	...
	switch (postbyte) {
		...
		case 0x88:
			IMMBYTE(EA);
			EA = X + SIGNED(EA);
			break;	/* this is a hack to make Vectrex work. It should be m68_state->icount-=1. Dunno where the cycle was lost :( */
		case 0x89:
			IMMWORD(EAP);
			EA += X;
			break;
...
static void
debugreg(cpu6809_t * p)
{
//  if((maincpu.pc <0xff40)||(maincpu.pc>0xff53)) return;
	printf("DEBUG: %04x %02x %02x %02x %02x EA=%04x CYCLE=%03d ", p->pc,
				 READB(p, p->pc), READB(p, p->pc + 1), READB(p, p->pc + 2), READB(p,
																																					p->
																																					pc +
																																					3),
				 p->ea, p->cycle);
