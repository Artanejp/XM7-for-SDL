...
static WORD     line_addr_old;	/* 直線補間
				 * 前回処理したVRAMアドレス */
 ...
 */
static void     FASTCALL
alu_writesub(WORD addr, BYTE dat)
{
    BYTE            temp;
 ...
 */
BOOL            FASTCALL
aluline_writeb(WORD addr, BYTE dat)
{
    DWORD           tmp;
