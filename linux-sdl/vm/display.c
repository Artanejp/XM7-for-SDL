...
#else
#define memcpy_200l(t,f,s) memmove(t,f,s)
#endif
/*
 *	グローバル ワーク
 */
...
fix_vram_address(void)
{
	...
	for (i = 0; i < 0x30000; i += 0x18000) {
		...
		memcpy_200l(&vram_c[0x10000 + i], &vram_c[0x0c000 + i], 0x4000);
		memcpy_200l(&vram_c[0x08000 + i], &vram_buf[0x04000], 0x4000);
		memcpy_200l(&vram_c[0x0c000 + i], &vram_buf[0x00000], 0x4000);
	}
