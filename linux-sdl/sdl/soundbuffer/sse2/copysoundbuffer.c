...
static inline void  _put_unaligned_int16(Sint16 *dst, v4hi v)
{
 ...
 #if defined(__SSE2__)
 int AddSoundBuffer_SSE2(Sint16 *dst, int rpos, Sint32 *opnsrc, Sint16 *beepsrc, Sint16 *cmtsrc, Sint16 *wavsrc[], int wavchannels, int samples)
 {
   int len1, len2;
   int i;
   int wavi;
...
//   _prefetch_data_read_l2(wav, sizeof(Sint16) * samples);
   for(i = 0; i < len1; i++) {
	   ...
	   vtmp = _get_unaligned_int16(wavp[wavi]);
           tt.vv = __builtin_ia32_paddsw128(tt.vv, vtmp.vv);
	   wavp[wavi] += 8;
	}
