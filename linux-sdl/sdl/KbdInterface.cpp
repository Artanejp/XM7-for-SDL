 ...
 * キーコードテーブルを初期値にする
 */
void KbdInterface::InitKeyTable(void){
	int i;
	for(i = 0; i<256 ; i++)
	{
 ...
 * キーテーブルを読み込む
 */
void KbdInterface::LoadKeyTable(void *pMap){
	int i;
...
void KbdInterface::InitLocalVar(void){
	kbd_snooped = FALSE;
}


void KbdInterface::ResetKeyMap(void)
{
	InitKeyTable();
}
...
void KbdInterface::CleanKbd(void)
{
//	KbdInterface::~KbdInterface();
}
...
Uint32 KbdInterface::GetKeyCode(Uint8 num)
{
	struct XM7KeyCode *q = KeyCodeTable2;
	int i;
...
Uint32 KbdInterface::GetKeyMod(Uint8 num)
{
	struct XM7KeyCode *q = KeyCodeTable2;
	int i;
...
struct XM7KeyCode *KbdInterface::GetKeyMap(Uint8 pushCode)
{
	// キーコードの変換
	struct XM7KeyCode *q = KeyCodeTable2;
	int i;
...
struct XM7KeyCode *KbdInterface::GetKeyMap(void)
{
	return KeyCodeTable2;
}

void KbdInterface::SetKeymap(Uint32 keyCode, Uint32 nativeCode, Uint32 keyMod)
{
	struct XM7KeyCode *q = KeyCodeTable2;
	Uint32 code = keyCode;
	Uint32 mod = keyMod;
...
void KbdInterface::SetKeymap(struct XM7KeyCode *p)
{
	struct XM7KeyCode *q = KeyCodeTable2;
	int i;
	...
	for(i = 0;i < 256; i++){
		...
		if(q[i].code == 0xffff) {// 終わりだよね
			 ...
			 * 終端コード新規
			 */
			q[i].pushCode = (Uint8)(p->pushCode & 0x00ff);
			q[i+1].code = (Uint32)0xffff;
			q[i+1].mod = (Uint32)0xffff;
			q[i].pushCode = 0xff;
			return;
...
void KbdInterface::SetResetKey(Uint32 code, Uint32 mod)
{
	ResetKey.sym = code;
	ResetKey.mod = mod;
...
void KbdInterface::SetMouseKey(Uint32 code, Uint32 mod)
{
	MouseCapture.sym = code;
