...
void FASTCALL GetDefMapKbd(local_sdlkeymap  *pMap, int mode)
{
	int i;
	int type;
        Uint16 num;
 ...
 *	キーマップ設定
 */
void FASTCALL SetMapKbd(local_sdlkeymap *pMap)
{
	ASSERT(pMap);
        int i;
	/* キーテーブル設定 */
        for (i=0; i<256 ; i++) {
	   if(pMap[i].keysym == 0xffff) break;
					...
					if (bKbdReal && (key_format != KEY_FORMAT_SCAN)) {
						if ((fm7 >= 0x3a) && (fm7 <= 0x46)) {
							...
							if (nLastKey2 && (nLastKey2 != key_last)) {
								...
								PushKeyCode(0xff);
//								PushKeyCode((BYTE)(nLastKey2 | 0x80));
								key_last = nLastKey2;
							}
						}
					}
...
BOOL OnMoveJoy(SDL_Event *event)
{
    ...
    //printf("JS:index = %d axis = %d num =  %d value = 0x%02x\n",index,axis,num,value);       
    /* JS番号は0,1のみ(暫定) */
    switch(num) 
     {
      case 0:
...
BOOL OnPressJoy(SDL_Event *event)
{
    ...
    //printf("JS:index = %d num =  %d value = 0x%02x\n",index,num,button);       
    /* JS番号は0,1のみ(暫定) */
    switch(num) 
     {
      case 0:
...
BOOL OnKeyPress(SDL_Event *event)
{
   ...
   for(i=0; i < 255 ; i++) {
        if(kbd_table[i].keysym == 0xffff) break;
